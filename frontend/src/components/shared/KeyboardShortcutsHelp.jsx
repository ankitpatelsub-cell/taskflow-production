...
export function KeyboardShortcutsHelp() {
  ...
  return (
    <>
      ...
      {open && (
        <>
          <div className="fixed inset-0 bg-gray-900/30 backdrop-blur-sm z-40" onClick={() => setOpen(false)} />
            ...
            <div className="space-y-2">
              {SHORTCUTS.map(({ key, desc }) => (
                <div key={key} className="flex items-center justify-between gap-4">
                  ...
                  <kbd className="px-2 py-0.5 bg-gray-100 dark:bg-slate-700 text-gray-800 dark:text-slate-200 text-xs font-mono rounded-xl border border-gray-200 dark:border-slate-600 shrink-0">
                    {key}
                  </kbd>
                </div>
              ))}
